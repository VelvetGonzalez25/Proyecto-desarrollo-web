...
class ContactShareController extends Controller
{
    ...
    public function store(Request $request)
    {
        $data = $request->validate([
            ...
            'user_email.not_in' => "No puedes compartir un contacto contigo mismo",
            'contact_email.exists' => 'Este contacto no se encontró en su lista de contactos.'
        ]);
        ...
        if ($shareExists) {
            return back()->withInput($request->all())->withErrors([
                'contact_email' => "Este contacto ya fue compartido con el usuario. $user->email",
            ]);
        }
